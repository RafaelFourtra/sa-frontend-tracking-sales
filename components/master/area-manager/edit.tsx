...
export const AreaManagerEdit = () => {
  const token = Cookies.get("auth_token");
  const [formData, setFormData] = useState<AreaManager>({
    NAME: "",
  });
  ...
  useEffect(() => {
    const fetchPermission = async () => {
      const permissionGranted = await checkPermission("area-manager.update");
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_AREA_MANAGER_EDIT_URL_API}${id}`,
        {
          cache: "no-store",
          headers: {
  ...
  const formik = useFormik<AreaManager>({
    ...
    onSubmit: async (values) => {
      if (navigator.onLine) {
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_AREA_MANAGER_UPDATE_URL_API}${id}`,
            {
              method: "PUT",
              headers: {
          ...
          if (result.status && result.message) {
            if (result.status == 200) {
              ...
              setTimeout(async () => {
                setIsLoading(false);
                router.push("/master/area-manager");
              }, 500);
            } else {
  ...
  return (
        <div className="px-4 lg:px-6 max-w-[95rem] bg-[#F5F6F8] mx-auto w-full h-full flex flex-col gap-4">
          ...
          <div className="flex justify-between flex-wrap gap-4 items-center">
            <h3 className="text-xl font-semibold">Edit Area Manager</h3>
