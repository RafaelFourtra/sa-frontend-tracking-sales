...
export const AreaManager = () => {
  ...
  const [areaManager, setAreaManager] = useState<AreaManager[]>([]);
  const { userLogin, checkPermission } = useAuthorization()
  const [access, setAccess] = useState(null)

  useEffect(() => {
    const fetchPermission = async () => {
      const permissionGranted = await checkPermission("area-manager.read");
  ...
  const filteredAM = React.useMemo(() => {
    return areaManager.filter((rm) => {
  ...
  }, [areaManager, formValues]);

  const pages =
    filteredAM.length > 0
      ? Math.ceil(filteredAM.length / rowsPerPage)
      : 1;

  const shortingRow = React.useMemo(() => {
    const data = [...filteredAM];
  ...
  const handleDelete = (id: any) => {
    ...
    swal({
      ...
      if (willDelete) {
        if (navigator.onLine) {
          const response = await fetch(`${process.env.NEXT_PUBLIC_AREA_MANAGER_DELETE_URL_API}${id}`, {
            method: 'DELETE',
            headers: {
  ...
  return (
        <div className="px-4 lg:px-6 max-w-[95rem] bg-[#F5F6F8] mx-auto w-full h-full flex flex-col gap-4">
          ...
          <div className="flex justify-between flex-wrap gap-4 items-center">
            <h3 className="text-xl font-semibold">All Area Manager</h3>
            <div className="flex flex-row gap-3.5 flex-wrap">
              <Button
                size="md"
                color="primary"
                onClick={() => router.push("/master/area-manager/create")}
              >
                Add Area Manager
              </Button>
            </div>
                              ...
                              <div>
                                <Tooltip content="Edit" color="foreground">
                                  <button
                                    onClick={() =>
                                      router.push(
                                        `/master/area-manager/edit/${item.ID}`
                                      )
                                    }
                                  >
