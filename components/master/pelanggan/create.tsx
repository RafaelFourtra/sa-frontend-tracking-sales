...
export const PelangganCreate = () => {
  ...
  const [filesPreview2, setFilesPreview2] = useState<FileWithPreview[]>([]);
  ...
  useEffect(() => {
    const fetchPermission = async () => {
      const permissionGranted = await checkPermission("pelanggan.create");
  ...
  const createDropzoneHandler = (setFilesPreview: any) => ({
    onDrop: (acceptedFiles: any) => {
      const validFiles = acceptedFiles.filter((file: any) => file.type.startsWith('image/'));
      ...
      if (navigator.onLine) {
        try {
          ...
          if (filesPreview.length > 0) {
            const file = filesPreview[0].file;;
            if (file.size > 307200) {
          ...
          if (filesPreview2.length > 0) {
            const file2 = filesPreview2[0].file;
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_PELANGGAN_CREATE_URL_API}`, {
            method: 'POST',
            headers: {
              "Authorization": `Bearer ${token}`,
            },
  ...
  return (
        <div className="px-4 lg:px-6 max-w-[95rem] bg-[#F5F6F8] mx-auto w-full h-full flex flex-col gap-4">
          ...
          <div className="flex justify-between flex-wrap gap-4 items-center">
            <h3 className="text-xl font-semibold">Create Pelanggan</h3>
            <div className="flex flex-row items-center gap-2.5 flex-wrap">
              <Button
                size="md"
                className="flex items-center bg-[#FFDD00]"
                onPress={() => router.push("/master/pelanggan")}
                    ...
                    <Tabs aria-label="Options" className="[--tab-highlight-color:#FFDD00]"
                      ...
                      <Tab key="termin" title="Termin">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          />
                          {detailTerm.map((item) => {
                            return <div key={item.TERMID} className="flex">
                      ...
                      <Tab key="penjualan" title="Penjualan">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            >
                              {tax.map((item: any) => {
                                return <Radio key={item.value?.toString()} value={item.value?.toString()}>{item.label}</Radio>
                              })}
                            </RadioGroup>
                          </div>
                          <Input
