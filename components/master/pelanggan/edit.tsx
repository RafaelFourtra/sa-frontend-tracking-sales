...
import {
  Button, Card, CardBody, CardFooter, Tabs,
  Tab,
  RadioGroup,
  Radio,
  Checkbox,
...
export const PelangganEdit = () => {
  ...
  useEffect(() => {
    const fetchPermission = async () => {
      const permissionGranted = await checkPermission("pelanggan.update");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (dataPelangganEdit.data?.FOTO_KTP) {
        ...
        setFilesPreview([{ preview: imageFotoKtpUrl, name: imageFotoKtpName, file: null, size: null }]);
      }
      if (dataPelangganEdit.data?.FOTO) {
        ...
        setFilesPreview2([{ preview: imageFotoUrl, name: imageFotoName, file: null, size: null }]);
      }
                    ...
                    <Tabs aria-label="Options" className="[--tab-highlight-color:#FFDD00]"
                      ...
                      <Tab key="termin" title="Termin">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          />
                          {detailTerm.map((item) => {
                            return <div className="flex" key={item.TERMID}>
                      ...
                      <Tab key="penjualan" title="Penjualan">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            >
                              {tax.map((item: any) => {
                                return <Radio value={item.value?.toString()} key={item.value?.toString()}>{item.label}</Radio>
                              })}
                            </RadioGroup>
                          </div>
                          <Input
