...
export const RegionalManagerCreate = () => {
  ...
  useEffect(() => {
    const fetchPermission = async () => {
      const permissionGranted = await checkPermission("regional-manager.create");
  ...
  const formik = useFormik<RegionalManagerData>({
    initialValues: {
      NAME: "",
    },
    ...
    onSubmit: async (values) => {
      if (navigator.onLine) {
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_REGIONAL_MANAGER_CREATE_URL_API}`, {
            method: 'POST',
            headers: {
  ...
  const handleChangeInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formik.setFieldValue(name as keyof RegionalManagerData, value);
  }
  ...
  return (
        <div className="px-4 lg:px-6 max-w-[95rem] bg-[#F5F6F8] mx-auto w-full h-full flex flex-col gap-4">
          ...
          <div className="flex justify-between flex-wrap gap-4 items-center">
            <h3 className="text-xl font-semibold">Create Regional Manager</h3>
