...
export const RegionalManager = () => {
  ...
  const [regionalManager, setRegionalManager] = useState<RegionalManager[]>([]);
  const { userLogin, checkPermission } = useAuthorization()
  const [access, setAccess] = useState(null)

  useEffect(() => {
    const fetchPermission = async () => {
      const permissionGranted = await checkPermission("regional-manager.read");
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (result.status == 200) {
        setRegionalManager(result.data || []);
  ...
  const filteredRM = React.useMemo(() => {
    return regionalManager.filter((rm) => {
  ...
  }, [regionalManager, formValues]);

  const pages =
    filteredRM.length > 0
      ? Math.ceil(filteredRM.length / rowsPerPage)
      : 1;

  const shortingRow = React.useMemo(() => {
    const data = [...filteredRM];
  ...
  const handleDelete = (id: any) => {
    ...
    swal({
      ...
      if (willDelete) {
        if (navigator.onLine) {
          const response = await fetch(`${process.env.NEXT_PUBLIC_REGIONAL_MANAGER_DELETE_URL_API}${id}`, {
            method: 'DELETE',
            headers: {
  ...
  return (
        <div className="px-4 lg:px-6 max-w-[95rem] bg-[#F5F6F8] mx-auto w-full h-full flex flex-col gap-4">
          ...
          <div className="flex justify-between flex-wrap gap-4 items-center">
            <h3 className="text-xl font-semibold">All Regional Manager</h3>
            <div className="flex flex-row gap-3.5 flex-wrap">
              <Button
                size="md"
                color="primary"
                onClick={() => router.push("/master/regional-manager/create")}
              >
                              ...
                              <div>
                                <Tooltip content="Edit" color="foreground">
                                  <button
                                    onClick={() =>
                                      router.push(
                                        `/master/regional-manager/edit/${item.ID}`
                                      )
                                    }
                                  >
