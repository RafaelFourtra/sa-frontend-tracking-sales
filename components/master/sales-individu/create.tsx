...
export const SalesIndividuCreate = () => {
  ...
  const formik = useFormik<SalesIndividuData>({
    initialValues: {
      NAME: "",
      SALESID: "",
      TIMSALESID: "",
    ...
    onSubmit: async (values) => {
      if (navigator.onLine) {
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_SALES_INDIVIDU_CREATE_URL_API}`, {
            method: 'POST',
            headers: {
  ...
  const handleChangeInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formik.setFieldValue(name as keyof SalesIndividuData, value);
  }
